...
fun main(args: Array<String>) {
    val input = "6046\t6349\t208\t276\t4643\t1085\t1539\t4986\t7006\t5374\t252\t4751\t226\t6757\t7495\t2923\n" +
    ...
    for (line in lines ) {
        ...
        val map = numbers.mapNotNull { s -> parseInt(s) }

        var foundNumber : Boolean = false
        var index1 = 0
        while (index1 < map.size) {
            var index2 = index1 + 1
            while (index2 < map.size) {
                if (map[index1] % map[index2] == 0)
                {
                    result += map[index1] / map[index2]
                    foundNumber = true
                    break
                }
                if (map[index2] % map[index1] == 0)
                {
                    result += map[index2] / map[index1]
                    foundNumber = true
                    break
                }
        ...
        if (!foundNumber)
        {
            println("No correct number not found")
        }
    }
